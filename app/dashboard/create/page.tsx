...
export default function AddTemplatePage() {
  ...
  const onSubmit = async (data: TemplateFormValues) => {
    ...
    try {
      ...
      if (!imageFilename) throw new Error("No image filename returned from server")
      ...
      const uploadedSvgFilenames = await Promise.all(
        svgFiles
          .filter(Boolean)
          .map(async (svgFile) => {
            ...
            if (!svgFilename) throw new Error("No SVG filename returned from server")
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem>
                    ...
                    <FormControl>
                      <Textarea placeholder="Describe the template..." rows={3} {...field} className="resize-none bg-secondary/50" />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
                ...
                render={() => (
                  <FormItem>
                    <FormLabel className="flex items-center gap-2">
                      ...
                      Template Preview Image<span className="text-destructive">*</span>
                    </FormLabel>
                    <Input type="file" accept=".jpg,.jpeg,.png" onChange={handleImageChange} className="bg-secondary/50 cursor-pointer" />
                    {imagePreview && (
                      <div className="mt-3 w-full max-w-sm">
                        <Image src={imagePreview} alt="Preview" width={400} height={300} className="rounded-lg border object-cover" />
                      </div>
                    )}
                  </FormItem>
                )}
                  ...
                  <FormItem>
                    ...
                    {svgFiles.map((_, index) => (
                      <div key={index} className="flex items-center gap-3 mb-2">
                        <Input type="file" accept=".svg" onChange={(e) => handleSvgChange(e, index)} className="bg-secondary/50 cursor-pointer" />
                        {svgFileNames[index] && <span className="text-sm text-muted-foreground truncate">{svgFileNames[index]}</span>}
                        {svgFiles.length > 1 && (
                          <button type="button" onClick={() => removeSvgInput(index)} className="text-destructive text-sm">
                            Remove
                          </button>
                        )}
                      </div>
                ...
                render={() => (
                  <FormItem>
                    <FormLabel className="flex items-center gap-2">
                      ...
                      PDF Documentation<span className="text-destructive">*</span>
                    </FormLabel>
                    <Input type="file" accept=".pdf" onChange={handlePdfChange} className="bg-secondary/50 cursor-pointer" />
                    {pdfFileName && (
