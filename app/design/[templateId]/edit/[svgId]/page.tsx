...
export default function SvgEditPage({ params }: Props) {
  ...
  const handlePreview = async () => {
    try {
      ...
      console.log(
        "New SVG content (first 200 chars):",
        newSvg.substring(0, 200)
      );
  ...
  const handleDownload = async () => {
    try {
      ...
      const loadingToats = toast.loading("Generating PDF...");
      ...
      if (result.downloadUrl) {
        ...
        toast.error(result.message, { id: loadingToats });
      } else {
        toast.error("faild to download.", { id: loadingToats });
